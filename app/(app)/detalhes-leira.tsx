...
export default function DetalhesLeiraScreen() {
  ...
  const handleRegistrarEnriquecimento = async () => {
    ...
    // Feedback inicial para o usuário não achar que travou
    ...
    try {
      ...
      // 2. Salvar no AsyncStorage (Celular)
      const enriquecimentosRegistrados = await AsyncStorage.getItem('leirasEnriquecimentos');
      ...
      // 3. Atualizar a Leira Localmente
      const leirasRegistradas = await AsyncStorage.getItem('leirasFormadas');
      const leirasData = leirasRegistradas ? JSON.parse(leirasRegistradas) : [];
      ...
      // 4. Lógica de Sincronização com Feedback Real
      await syncService.adicionarFila('enriquecimento', novoEnriquecimento);

      // Verifica internet
      const temInternet = await syncService.verificarInternet();
      let tituloAlert = 'Salvo Offline ✈️';
      let msgAlert = `${pesoAdicionado} ton salvas no celular. Enviarei quando conectar.`;

      if (temInternet) {
        // Tenta sincronizar e pega o resultado (true/false)
        const sucesso = await syncService.sincronizar();

        if (sucesso) {
          tituloAlert = 'Sucesso Total! ✅';
          msgAlert = `${pesoAdicionado} ton salvas e SINCRONIZADAS com o banco!`;
        } else {
          tituloAlert = 'Atenção ⚠️';
          msgAlert = `${pesoAdicionado} ton salvas no celular, mas houve erro ao enviar para o servidor.`;
        }
      }
      ...
      // 5. Exibe o alerta final com o status real
      Alert.alert(tituloAlert, msgAlert);
