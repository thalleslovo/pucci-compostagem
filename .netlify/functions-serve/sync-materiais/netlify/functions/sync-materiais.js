...
var handler = async (event) => {
  console.log("\u{1F504} Fun\xE7\xE3o sync-materiais acionada");
  ...
  try {
    ...
    console.log(`\u{1F4E4} Recebido: ${materiais.length} materiais do operador ${operadorNome}`);
    ...
    for (const material of materiais) {
      try {
        console.log(`\u{1F4AA} Processando material: ${material.id}`);
        const { data, error } = await supabase.from("materiais_registrados").insert({
          id: material.id,
    ...
    console.log(`\u2705 Sincroniza\xE7\xE3o conclu\xEDda: ${sincronizados}/${materiais.length} inseridos`);
    return {
      statusCode: 200,
      body: JSON.stringify({
        sucesso: true,
