...
var handler = async (event) => {
  ...
  try {
    ...
    if (materiais.length > 0) {
      console.log(`\u{1F575}\uFE0F DEBUG - Processando ${materiais.length} itens.`);
    ...
    for (const material of materiais) {
      if (material.deletado === true) {
        const { error: error2 } = await supabase.from("materiais_registrados").delete().eq("id", material.id);
        if (!error2) deletados++;
        continue;
      }
    ...
    return {
      ...
      body: JSON.stringify({
        ...
        sincronizados,
        deletados,
        mensagem: `Processado: ${sincronizados} salvos, ${deletados} exclu\xEDdos.`
      })
    };
  } catch (error) {
