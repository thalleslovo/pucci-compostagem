...
// netlify/functions/sync-clima.ts
var sync_clima_exports = {};
__export(sync_clima_exports, {
  handler: () => handler
});
module.exports = __toCommonJS(sync_clima_exports);
...
var handler = async (event) => {
  console.log("\u{1F504} Fun\xE7\xE3o sync-clima acionada");
  ...
  try {
    ...
    console.log(`\u{1F4E4} Recebido: ${clima.length} registros de clima do operador ${operadorNome}`);
    ...
    for (const registro of clima) {
      try {
        console.log(`\u{1F4AA} Processando clima da leira: ${registro.leiraId}`);
        console.log(`\u{1F50D} DEBUG - Usando operadorId: ${operadorId}`);
        const { data, error } = await supabase.from("clima_leira").insert({
          id: registro.id,
    ...
    console.log(`\u2705 Sincroniza\xE7\xE3o conclu\xEDda: ${sincronizados}/${clima.length} inseridos`);
    return {
      statusCode: 200,
      body: JSON.stringify({
        sucesso: true,
...
//# sourceMappingURL=sync-clima.js.map
